...
(() => {
  ...
  function updateHUDStats() {
    ...
    const hudStudents = document.getElementById("hud-students");
    const hudGroups = document.getElementById("hud-groups");
    const hudMentors = document.getElementById("hud-mentors");

    if (hudStudents) hudStudents.textContent = students.length + "+";
    if (hudGroups) hudGroups.textContent = groups.length;
    if (hudMentors) hudMentors.textContent = mentors.length;
  }
  ...
  function renderCoursesFromDB() {
    ...
    const courses = db.courses.getAll().filter(c => c.status === "active");
    if (!courses.length) return;

    // Keep the smart-card (tavsiya tizimi) at the end
    const smartCard = grid.querySelector(".smart-card");

    // Clear existing course cards (but not smart-card)
    const existingCards = grid.querySelectorAll(".course-card");
    existingCards.forEach(card => card.remove());

    // Create dynamic course cards from DB
    courses.forEach(course => {
      ...
      const levelText = levelMap[course.level] || course.level || "Boshlang'ich";

      const card = document.createElement("article");
      card.className = "course-card";
      card.setAttribute("data-level", course.level || "beginner");
      card.setAttribute("data-course-name", course.name);

      card.innerHTML = `
        <h3>${course.name}</h3>
        <div class="meta">${levelText}</div>
        <div class="meta">Davomiyligi: ${course.duration || "3 oy"}</div>
        <div class="desc">${course.description || ""}</div>
        <div class="course-price">${Number(course.price || 0).toLocaleString()} so'm/oy</div>
        <div class="chips">
          <span class="chip">Portfolio</span>
          <span class="chip">Amaliyot</span>
          <span class="chip">Mentor</span>
        </div>
        <button
          class="btn btn-ghost course-details-btn"
      ...
      // Insert before smart-card
      if (smartCard) {
        grid.insertBefore(card, smartCard);
      } else {
  ...
  function renderGroupsFromDB() {
    ...
    // Get all students to calculate actual counts
    ...
    groups.forEach(g => {
      ...
      // Calculate REAL student count from students collection (exclude pending/rejected)
      ...
      chip.innerHTML = `
        ...
        <div class="group-attendance">
          ...
          <div class="group-attendance-text">
            ...
            <span>${realStudentCount} talaba</span>
          </div>
        </div>
      `;
  ...
  function initDBSync() {
    ...
    setInterval(() => {
      ...
      if (currentTime !== lastSyncTime) {
        ...
        renderGroupsFromDB();
        updateHUDStats();
        console.log("🔄 Ma'lumotlar yangilandi:", new Date().toLocaleTimeString());
      }
    }, 3000);
  }
