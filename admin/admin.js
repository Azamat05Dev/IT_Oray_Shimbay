...
// Hybrid: MongoDB API + LocalStorage fallback

// ==============================
// YORDAMCHI FUNKSIYALAR
// ==============================

// Hybrid API Request - First tries MongoDB API, falls back to LocalStorage
async function adminApiRequest(path, options = {}) {
  ...
  // Try MongoDB API if AdminAPI is available and online
  if (window.AdminAPI && navigator.onLine) {
    try {
      // Map paths to AdminAPI methods
      if (path === "/courses" && method === "GET") {
        const result = await AdminAPI.courses.getAll();
        if (result.success && result.courses) {
          return result.courses;
        }
      }
      else if (path === "/students" && method === "GET") {
        const result = await AdminAPI.students.getAll();
        if (result.success && result.students) {
          return result.students.map(s => ({
            id: s._id || `ST-${String(s.id).padStart(4, '0')}`,
            fullName: s.fullName,
            phone: s.phone,
            email: s.email,
            course: s.courseName || s.courseId,
            group: s.groupId,
            status: getStatusLabel(s.status),
            paymentStatus: getPaymentLabel(s.paymentStatus)
          }));
        }
      }
        ...
        if (result.success) return result.applications;
      }
      else if (path === "/groups" && method === "GET") {
        const result = await AdminAPI.groups.getAll();
        if (result.success) return result.groups;
      }
      else if (path === "/mentors" && method === "GET") {
        const result = await AdminAPI.mentors.getAll();
        if (result.success) return result.mentors;
      }
      else if (path === "/payments" && method === "GET") {
        const result = await AdminAPI.payments.getAll();
        if (result.success) return result.payments;
      }
    } catch (apiError) {
      if (window.AdminAPI) console.warn('API error, using LocalStorage:', apiError.message);
    }
  }

  // Fallback to LocalStorage
  return localStorageRequest(path, method, body);
}

// Helper functions for status labels
function getStatusLabel(status) {
  const labels = {
    'active': 'Aktiv', 'applied': 'Kutilmoqda', 'frozen': 'Muzlatilgan',
    'graduated': 'Bitirgan', 'rejected': 'Rad etilgan'
  };
...
function getPaymentLabel(status) {
  const labels = {
    'paid': "To'langan", 'partial': 'Qisman', 'unpaid': 'Qarzdor', 'debt': 'Qarzdor'
  };
  return labels[status] || status;
}

// LocalStorage fallback
function localStorageRequest(path, method, body) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Route handling - Original LocalStorage logic
      if (path === "/analytics" && method === "GET") {
        ...
        resolve([
          { title: "Yangi tez ariza", text: "Test • Frontend", time: "Bir necha soniya oldin", unread: true }
        ]);
      }
  ...
  if (quickSyncBtn) {
    quickSyncBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const response = await AdminAPI.students.getAll({ limit: 1000 });
        ...
        if (response.success && response.students) {
          ...
          if (db && db.students) {
            ...
            const existingIds = db.students.getAll().map(s => s.id);
            existingIds.forEach(id => db.students.delete(id));

            // Add all students from backend
            response.students.forEach(student => {
              // Convert MongoDB _id to simple id
              const studentData = {
                ...student,
          ...
          adminShowToast(`✅ ${response.students.length} ta talaba sinxronlandi`, "success");

          // Reload all data
          if (window.loadDashboardAnalytics) loadDashboardAnalytics();
          if (window.loadStudents) window.loadStudents();
          if (window.reloadAllData) reloadAllData();
        } else {
          adminShowToast(response.message || "Sinxronlashda xatolik.", "error");
        }
      } catch (err) {
        console.error('Sync error:', err);
        adminShowToast(err.message || "Server bilan bog'lanishda xatolik.", "error");
      } finally {
...
IT Center Portal
  ...
  // ==============================
  // BULK ACTIONS - REMOVED (moved to lines 5740+)
  // ==============================
  // The bulk action handlers have been moved to the end of the file (lines 5740-6046)
  // to use the new DOM-based selection system and backend API integration.
  ...
  function saveAttendance() {
    ...
    const totalStudents = checkboxes.length; // Use actual student count from DOM
    let presentCount = 0;
    ...
    const result = db.attendance.markAttendance(
      ...
      presentCount,
      totalStudents // Use real student count, not static studentCount
    );

    // Also update the group's studentCount in the database for sync
    db.groups.update(currentAttendanceGroup.id, { studentCount: totalStudents });

    adminShowToast(`✅ Davomat saqlandi: ${presentCount}/${totalStudents} (${result.percentage}%)`, "success");
    loadTodayStats();
  }
  ...
  // Expose to window for admin-students.js
  ...
  // ============================================
  // BULK ACTIONS FOR STUDENTS SECTION
  // ============================================

  // Get selected student IDs - query DOM directly for checked checkboxes
  function getSelectedStudentIds() {
    const checked = document.querySelectorAll('.student-checkbox:checked');
    return Array.from(checked).map(cb => {
      // Get ID from data-id attribute on the checkbox
      return cb.dataset.id || null;
    }).filter(Boolean);
  }
  ...
  // Listen to checkbox changes in students table
  document.getElementById('students-table')?.addEventListener('change', (e) => {
  ...
  document.getElementById('bulk-delete')?.addEventListener('click', async () => {
    ...
    if (!confirm(`${ids.length} ta talabani o'chirishni tasdiqlaysizmi?`)) return;

    try {
      const response = await AdminAPI.students.bulkDelete(ids);

      if (response.success) {
        adminShowToast(`✅ ${response.results.filter(r => r.success).length} ta talaba o'chirildi`, 'success');
        ...
        if (db) {
          ids.forEach(id => {
            ...
            if (!isNaN(numId)) {
              db.students.delete(numId);
            }
          });
          db.logActivity('bulk_delete', 'Admin', `${ids.length} ta talaba o'chirildi`);
        }
        ...
        adminShowToast(`❌ ${response.message || "O'chirishda xatolik"}`, 'error');
      }
    } catch (error) {
      console.error('Bulk delete error:', error);
      adminShowToast('❌ Server bilan bog\'lanishda xatolik', 'error');
    }
  });

  // BULK SMS
  document.getElementById('bulk-send-sms')?.addEventListener('click', async () => {
    const ids = getSelectedStudentIds();
    if (ids.length === 0) {
      adminShowToast('❌ Avval talabalarni tanlang', 'error');
      return;
    ...
    // Get phone numbers of selected students
    const phones = ids.map(id => {
      const student = db.students.getById(parseInt(id));
      return student?.phone;
    }).filter(Boolean);

    if (phones.length === 0) {
      adminShowToast('❌ Tanlangan talabalarda telefon raqam yo\'q', 'error');
      return;
    }
    ...
    modal.innerHTML = `
      <div class="admin-modal-content" style="max-width: 500px;">
        <div class="admin-modal-header">
          <h2>📱 SMS yuborish (${phones.length} ta)</h2>
          <button class="admin-modal-close" onclick="this.closest('.admin-modal-overlay').remove()">✕</button>
        </div>
        <div class="admin-modal-body">
          <div class="admin-form-group">
            <label>Xabar matni</label>
            <textarea id="sms-message" class="admin-input" rows="4" placeholder="SMS matnini kiriting..."></textarea>
          </div>
          <div style="font-size: 0.85rem; color: var(--text-secondary);">
            Telefon raqamlar: ${phones.slice(0, 3).join(', ')}${phones.length > 3 ? '...' : ''}
          </div>
        </div>
        <div class="admin-modal-footer">
          <button class="admin-btn" onclick="this.closest('.admin-modal-overlay').remove()">Bekor qilish</button>
          <button class="admin-btn admin-btn-primary" id="confirm-sms-btn">📱 Yuborish</button>
        </div>
      </div>
    `;
    document.body.appendChild(modal);

    document.getElementById('confirm-sms-btn').onclick = async () => {
      const message = document.getElementById('sms-message').value.trim();
      if (!message) {
        adminShowToast('❌ Xabar matnini kiriting', 'error');
        return;
      }

      try {
        const response = await AdminAPI.students.bulkSendSMS(ids, message);

        if (response.success) {
          adminShowToast(`✅ SMS yuborildi: ${response.sent} ta raqamga`, 'success');
          db.logActivity('bulk_sms', 'Admin', `${response.sent} ta talabaga SMS: "${message.substring(0, 50)}..."`);
          modal.remove();
        } else {
          adminShowToast(`❌ ${response.message || "SMS yuborishda xatolik"}`, 'error');
        }
      } catch (error) {
  ...
  // BULK CHANGE STATUS
  document.getElementById('bulk-change-status')?.addEventListener('click', () => {
    const ids = getSelectedStudentIds();
    if (ids.length === 0) {
      adminShowToast('❌ Avval talabalarni tanlang', 'error');
      return;
    ...
    modal.innerHTML = `
      <div class="admin-modal-content" style="max-width: 400px;">
        <div class="admin-modal-header">
          <h2>📋 Status o'zgartirish (${ids.length} ta)</h2>
          <button class="admin-modal-close" onclick="this.closest('.admin-modal-overlay').remove()">✕</button>
        </div>
        <div class="admin-modal-body">
          <div class="admin-form-group">
            <label>Yangi status</label>
            <select id="new-status-select" class="admin-select">
              <option value="active">✅ Faol</option>
        ...
        <div class="admin-modal-footer">
          <button class="admin-btn" onclick="this.closest('.admin-modal-overlay').remove()">Bekor qilish</button>
          <button class="admin-btn admin-btn-primary" id="confirm-status-btn">✅ Saqlash</button>
        </div>
      </div>
    `;
    document.body.appendChild(modal);
    ...
    document.getElementById('confirm-status-btn').onclick = async () => {
      ...
      try {
        const response = await AdminAPI.students.bulkChangeStatus(ids, newStatus);

        if (response.success) {
          adminShowToast(`✅ ${response.updated} ta talabaning statusi o'zgartirildi`, 'success');
          ...
          if (db) {
            ids.forEach(id => {
              const numId = parseInt(id);
              if (!isNaN(numId)) {
                db.students.update(numId, { status: newStatus });
              }
            });
            db.logActivity('bulk_status', 'Admin', `${response.updated} ta talabaga yangi status: ${newStatus}`);
          }
          ...
          adminShowToast(`❌ ${response.message || "Status o'zgartirishda xatolik"}`, 'error');
        }
      } catch (error) {
  ...
  // BULK CHANGE GROUP
  document.getElementById('bulk-change-group')?.addEventListener('click', () => {
    const ids = getSelectedStudentIds();
    if (ids.length === 0) {
      adminShowToast('❌ Avval talabalarni tanlang', 'error');
      return;
    ...
    const groups = db.groups?.getAll() || [];
    const groupOptions = groups.map(g => `<option value="${g.id}">${g.id} - ${g.room || 'N/A'}</option>`).join('');
    ...
    modal.innerHTML = `
      <div class="admin-modal-content" style="max-width: 400px;">
        <div class="admin-modal-header">
          <h2>👥 Guruh o'zgartirish (${ids.length} ta)</h2>
          <button class="admin-modal-close" onclick="this.closest('.admin-modal-overlay').remove()">✕</button>
        </div>
        <div class="admin-modal-body">
          <div class="admin-form-group">
            <label>Mavjud guruhdan tanlash</label>
            <select id="new-group-select" class="admin-select">
              <option value="">-- Tanlang --</option>
              ${groupOptions}
            </select>
          </div>
          <div class="admin-form-group" style="margin-top: 16px;">
            <label>Yoki yangi guruh ID kiriting</label>
            <input type="text" id="new-group-input" class="admin-input" placeholder="Masalan: F-15">
          </div>
        </div>
        <div class="admin-modal-footer">
          <button class="admin-btn" onclick="this.closest('.admin-modal-overlay').remove()">Bekor qilish</button>
          <button class="admin-btn admin-btn-primary" id="confirm-group-btn">✅ Saqlash</button>
        </div>
      </div>
    `;
    document.body.appendChild(modal);
    ...
    document.getElementById('confirm-group-btn').onclick = async () => {
      const selectedGroup = document.getElementById('new-group-select').value;
      const newGroupInput = document.getElementById('new-group-input').value.trim();
      const newGroupId = newGroupInput || selectedGroup;

      if (!newGroupId) {
        adminShowToast('❌ Guruh tanlang yoki yangi ID kiriting', 'error');
        return;
      }

      try {
        const response = await AdminAPI.students.bulkChangeGroup(ids, newGroupId);

        if (response.success) {
          adminShowToast(`✅ ${response.updated} ta talaba ${newGroupId} guruhiga o'tkazildi`, 'success');
          ...
          if (db) {
            ids.forEach(id => {
              const numId = parseInt(id);
              if (!isNaN(numId)) {
                db.students.update(numId, { groupId: newGroupId });
              }
            });
            db.logActivity('bulk_group', 'Admin', `${response.updated} ta talaba ${newGroupId} ga o'tkazildi`);
          }
          ...
          adminShowToast(`❌ ${response.message || "Guruh o'zgartirishda xatolik"}`, 'error');
        }
      } catch (error) {
  ...
  // ============================================
  // TEZ ARIZALAR (APPLICATIONS) TABLE
  // ============================================

  const applicationsStatusLabels = {
    'new': { text: '🆕 Yangi', class: 'admin-status-new' },
    'called': { text: '📞 Qo\'ng\'iroq qilindi', class: 'admin-status-active' },
    'enrolled': { text: '✅ Guruhga qo\'shilgan', class: 'admin-status-success' },
    'closed': { text: '🚫 Yopilgan', class: 'admin-status-frozen' }
  };
  ...
  function renderApplicationsTable() {
    ...
    // Get students with 'applied' status (quick applications)
    const freshData = JSON.parse(localStorage.getItem('itcenter_db') || '{}');
    let students = (freshData.students || []).filter(s => s.status === 'applied');

    // Sort by newest first
    students.sort((a, b) => new Date(b.applicationDate || b.enrollDate || 0) - new Date(a.applicationDate || a.enrollDate || 0));

    if (students.length === 0) {
      tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 40px; color: var(--text-secondary);">✅ Hozircha yangi ariza yo\'q</td></tr>';
      return;
    }

    tbody.innerHTML = students.map(s => {
      const appTime = s.applicationDate ? new Date(s.applicationDate).toLocaleTimeString('uz-UZ', { hour: '2-digit', minute: '2-digit' }) : '—';
      const courseNames = { 1: 'Frontend', 2: 'Python', 3: 'Kids IT', 4: 'UI/UX', 5: 'Mobile' };
      const courseName = courseNames[s.courseId] || s.courseId || 'N/A';
      const format = s.educationFormat === 'online' ? 'Onlayn' : 'Offlayn';
      return `
        <tr data-app-id="${s.id}">
          <td>${appTime}</td>
          <td><strong>${s.fullName}</strong></td>
          <td>${s.phone || s.email || '—'}</td>
          <td><span class="admin-badge admin-badge-info">${courseName}</span></td>
          <td>${format}</td>
          <td><span class="admin-status admin-status-new">🆕 Yangi</span></td>
          <td class="admin-actions-cell">
            <button class="admin-btn admin-btn-sm" onclick="markAppCalled(${s.id})" title="Qo'ng'iroq qilindi">📞</button>
            <button class="admin-btn admin-btn-sm admin-btn-primary" onclick="openApproveModal(${s.id})" title="Guruhga qo'shish">✅</button>
            <button class="admin-btn admin-btn-sm admin-btn-danger" onclick="closeApp(${s.id})" title="Yopish">🚫</button>
          </td>
        </tr>
      `;
    }).join('');
  ...
  // Mark application as called
  window.markAppCalled = function (studentId) {
    const db = window.ITCenterDB;
    if (!db) return;
  ...
  // Close application
  window.closeApp = function (studentId) {
    if (!confirm('Arizani yopishni tasdiqlaysizmi?')) return;
    ...
    db.students.update(studentId, {
      status: 'rejected',
      rejectionReason: 'Admin tomonidan yopildi',
      rejectedAt: new Date().toISOString()
    });

    adminShowToast('🚫 Ariza yopildi', 'success');
    renderApplicationsTable();
    renderRegistrationsTable(); // Also update registrations
  };

  // Load applications when section is shown
  const applicationsSection = document.getElementById("admin-section-applications");
  if (applicationsSection) {
    ...
    observer.observe(applicationsSection, { attributes: true, attributeFilter: ["style"] });
  }

  // Initial load for applications if visible
  renderApplicationsTable();
